...
test("conserva las entidades y recuentos del catálogo de Ciencias", () => {
  ...
  assert.equal(catalog.subjects[0].id, "ciencias");
  assert.equal(catalog.topics.length, 11);
  assert.equal(catalog.activities.length, 65);
  assert.equal(catalog.activities.reduce((total, activity) => total + activity.topicIds.length, 0), 69);
  assert.equal(catalog.activities.filter((activity) => activity.topicIds.length > 1).length, 4);
  assert.deepEqual(report.byPlatform, { educaplay: 43, wordwall: 22 });
  assert.deepEqual(report.byLanguage, { en: 39, es: 26 });
  assert.deepEqual(report.bySourceKind, { external: 65 });
});

test("conserva exactamente los campos públicos y su significado", () => {
  assert.deepEqual(new Set(newById.keys()), new Set(legacy.activities.map((activity) => activity.id)));
